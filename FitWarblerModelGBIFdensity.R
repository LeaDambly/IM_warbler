...
load("Data/Stacks.RData")
  ...
  # create priors 
  C.F. <- list(mean = list(int.gbif = 0),
               mean.intercept=0, prec.intercept = 0.001,
               prec = list(int.gbif = 1))
  ...
  form <- formula(resp ~ 0 + elevation + canopy + Intercept + X + Y + int.gbif + log(density) + f(i, model = spde))
  ...
  save(C.F., spde, form, warbler_model_gbif, file="Data/WarblerModelOutputGBIFdensity.RData")
