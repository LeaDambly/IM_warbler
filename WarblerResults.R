...
par(mfrow=c(1,2), mar=c(2.1,2,3,1), oma=c(2,2,0,0))
plot(warbler_model$model$marginals.fixed$elevation, type="l", main="Elevation", xlab="")
abline(v=0,col="red")
plot(warbler_model$model$marginals.fixed$canopy, type="l", main="Canopy", xlab="")
abline(v=0,col="red")
...
plot(PA)
points(BBA_sp[which(BBA_sp@data$present == FALSE),], cex=0.4, col="pink", pch=16) # Add BBA
points(BBA_sp[which(BBA_sp@data$present == TRUE),], cex=0.4, col="red3", pch=16)
points(BBS_sp, cex=0.5, col="blue", pch=16) # Add BBS
points(eBird_sp, cex=0.5, col="sandybrown", pch=16) # Add eBird
legend(-79, 42.45, c("BBA, absent", "BBA, present", "BBS route", "eBird"), col=c("pink", "red3", "blue", "sandybrown"), pch=16, 
       ncol = 2)
dev.off()
