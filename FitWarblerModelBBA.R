...
load("Data/Stacks.RData")
  ...
  form <- formula(resp ~ 0 + elevation + canopy + Intercept + X + Y + int.BBA + 
                    f(i, model = spde))
  ...
  save(C.F., spde, form, warbler_model_BBA, file="Data/WarblerModelOutputBBS.RData")
