...
if(!file.exists("Data/BBS.csv")) {
  ...
  BBS_Wren <- GetRouteData(AOU = 6540,
                           countrynum = 840,
                           states = PACode,
                           year = PAYears,
...
# eBird, downloaded from GBIF
 if(!file.exists("Data/eBird.csv")) {
   ...
   eBird.raw <- spocc::occ("Setophaga caerulescens",
                           from = "gbif",
...
NLCD_canopy <- get_nlcd(template = PA,
                        year = 2011,
                        dataset = "canopy",
                        label = "PA_lc")
...
covariates <- SpatialPointsDataFrame(covariate_coords, 
                                     data = covariate_data,
                                     proj4string = crs(proj))
...
# Save the data
save(proj, PA, BBA_sp, BBS_sp, eBird_sp, covariates, covariates_eBird,
